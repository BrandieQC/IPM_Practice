...
wl2_all_2023_timeprep %>% filter(surv==0)
wl2_all_2023_timeprep %>% filter(BedLoc=="C_27_B")
wl2_all_2023 %>% filter(BedLoc=="C_27_B")

wl2_all_2023 %>% 
  mutate(death.date=mdy(death.date)) %>% 
  filter(survey_date==death.date) %>% 
...
wl2_all_2023 %>% 
  ...
  filter(survey_date==death.date) %>% 
  filter(!is.na(height.cm))
#sometimes we have a height on the day of death and sometimes we don't...
```
