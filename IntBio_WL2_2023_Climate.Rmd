...
-   Try tidymodels for cross validation 

-   Add in climate data

    -   Use moisture and temp for the time interval between t and t+1

    -   Keep SWC and Temp in separate models b/c negatively correlated (r>-0.69 for everything except min temp: r=-0.38)

    -   Think about ways to aggregate

        -   See chillR package for Chilling Hours, Chill Units, Chil Portions, Growing Degree Hours, and Growing Degree Days
...
-   Think about how you want to summarize the data.

    -   See chillR package - for chilling and heat 

    -   Diurnal range, photothermal units, hours below or above a
...
```{r}
wl2_all_2023_clim <- wl2_all_2023_timeprep_scaled %>% 
  ...
  filter(!is.na(Interval))
head(wl2_all_2023_clim)
summary(wl2_all_2023_clim)
names(wl2_all_2023_clim)
```
...
surv.rec <- wl2_all_2023_clim %>% 
  select(surv, height.cm, week, elapsed_weeks, parent.pop:mf, pop.mf, block, meanTemp:meanSWC) %>% 
  recipe() %>% 
...
surv.models <- tibble(wflow=list(
   ...
   m2_min_temp = {surv.wflow %>%
      add_model(surv.spec,
                formula = surv ~ height.cm + minTemp + (1|week) + (1|block) + (1|parent.pop/mf))},
   m3_max_temp = {surv.wflow %>%
      add_model(surv.spec,
                formula = surv ~ height.cm + maxTemp + (1|week) + (1|block) + (1|parent.pop/mf))},
   m4_avg_temp = {surv.wflow %>%
      add_model(surv.spec,
                formula = surv ~ height.cm + meanTemp + (1|week) + (1|block) + (1|parent.pop/mf))},
   m5_diurnal_range = {surv.wflow %>%
      add_model(surv.spec,
                formula = surv ~ height.cm + meanDiurnal_Range + (1|week) + (1|block) + (1|parent.pop/mf))},
   m6_swc = {surv.wflow %>%
      add_model(surv.spec,
                formula = surv ~ height.cm + meanSWC + (1|week) + (1|block) + (1|parent.pop/mf))}
),
name = names(wflow))
```
...
system.time( { 
  surv.models <- surv.models %>%
    mutate(fit = future_map(wflow, fit, data = drop_na(wl2_all_2023_clim,contains("Temp"), contains("Diurnal"),
...
#one model with convergence issues 
...
surv.models.best <- surv.models %>%
  unnest(glance) %>%
  filter(rank(BIC) <= 3 | str_detect(name, "ml1_linear")) %>%
  arrange(BIC)
...
```{r}
set.seed(1001)
surv_folds <- vfold_cv(wl2_all_2023_clim, v = 10)
...
### Predictions 
```{r}
newdata.survival <- wl2_all_2023_clim %>%
...
#Error in UseMethod("extract_fit_engine") : 
#  no applicable method for 'extract_fit_engine' applied to an object of class "c('glmerMod', 'merMod')"

surv.predictions %>%
  ungroup() %>%
  nest(.by = parent.pop) %>% 
  mutate(plot = map2(data, parent.pop, \(x, pp) {
    ggplot(x, aes(x=height.cm, y = surv)) +
      geom_smooth(color = "grey50", lty=3, se = FALSE, method = "gam", data = {wl2_all_2023_clim %>% filter(parent.pop==pp)}) + 
      geom_line() +
      facet_wrap(~week, scale = "free_x") +
      geom_point(alpha = 0.3, data = {wl2_all_2023_clim %>% filter(parent.pop==pp)}) + 
      ggtitle(pp) +
...
growth.rec <-  wl2_all_2023_clim %>% 
  select(logHeight_next, height.cm, week, elapsed_weeks, parent.pop:mf, pop.mf, block, meanTemp:meanSWC) %>% 
  recipe() %>% 
  update_role(logHeight_next, new_role = "outcome") %>% 
  update_role(c(height.cm, week, elapsed_weeks, parent.pop, mf, pop.mf, block, 
                contains("mean"), maxTemp, minTemp), new_role = "predictor")
...
growth.models <- tibble(wflow=list(
   m1_linear_weeks = {growth.wflow %>%
      add_model(growth.spec,
                formula = logHeight_next ~ height.cm + (height.cm|parent.pop) + (1|week) + (1|pop.mf) + (1|block)) },
   m2_min_temp = {growth.wflow %>%
      add_model(growth.spec,
                formula = logHeight_next ~ height.cm + minTemp + (height.cm|parent.pop) + (1|week) + (1|pop.mf) + (1|block))},
   m3_max_temp = {growth.wflow %>%
      add_model(growth.spec,
                formula = logHeight_next ~ height.cm + maxTemp + (height.cm|parent.pop) + (1|week) + (1|pop.mf) + (1|block))},
   m4_avg_temp = {growth.wflow %>%
      add_model(growth.spec,
                formula = logHeight_next ~ height.cm + meanTemp + (height.cm|parent.pop) + (1|week) + (1|pop.mf) + (1|block))},
   m5_diurnal_range = {growth.wflow %>%
      add_model(growth.spec,
                formula = logHeight_next ~ height.cm + meanDiurnal_Range + (height.cm|parent.pop) + (1|week) + (1|pop.mf) + (1|block))},
   m6_swc = {growth.wflow %>%
      add_model(growth.spec,
                formula = logHeight_next ~ height.cm + meanSWC + (height.cm|parent.pop) + (1|week) + (1|pop.mf) + (1|block))}
),
name = names(wflow))
```
...
system.time( { # fast
  growth.models <- growth.models %>%
    mutate(fit = map(wflow, fit, data = drop_na(wl2_all_2023_clim, contains("Temp"), contains("Diurnal"),
                                                       contains("SWC") )),
           glance = map(fit, glance)
    ) 
})
...
## P matrix with weekly climate (need to update this code, mostly copied from Julin's code and my old code for now )
