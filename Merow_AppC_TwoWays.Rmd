...
#min.size=.9*min(c(d94$size, d94$sizeNext),na.rm=T) #code from App A
...
### Merow # Fruits models - Need to filter to fec0==1??
...
### BQC Version - Number Fruits/Plant - Need to filter to fec0==1??
```{r}
...
p.flower.x=function(x,params.rep) { #flowering prob. 
              u=exp(params.rep$flower.int+params.rep$flower.slope*x+params.rep$flower.slope3*x^3)
              return(u/(1+u)) #back transform from logit scale 
}

#reproduction function (with above prob. function incorporated)
f.yx=function(xp,x,params.rep) {
        p.flower.x(x,params.rep)* #have to flower first
    exp(params.rep$fruit.int+params.rep$fruit.slope*x+params.rep$fruit.slope3*x^3)* #then produce a certain number of fruits 
    ...
    params.rep$germ.prob* #those seeds have to germinate
              ...
              dnorm(xp,mean=params.rep$recruit.size.mean,sd=params.rep$recruit.size.sd) #recruits grow to a certain size 
}
```
...
Get lots of NAs at bigger sizes!

## Consider transitions in and out of the discrete stage 
```{r}
dto94 = new("discreteTrans") #this is different from the appendix b/c "makeDiscreteTrans" was not working"
...
dto94@moveToDiscrete = glm(rep(0, 21) ~ 1, family = binomial) #this is different from the appendix b/c of the first issue above 
dtrans = as.matrix(staySB + (1 - staySB)*fec3*fec4)
dimnames(dtrans) = list("continuous","continuous")
dto94@discreteTrans = dtrans
```
...
BQC: Will get an error for this until fix the NAs in the F matrix 
```{r}
K=P+F                             # full matrix
